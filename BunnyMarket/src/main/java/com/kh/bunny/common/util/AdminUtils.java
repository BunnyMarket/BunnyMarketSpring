...
public class AdminUtils {
	...
	public static String getPageBar(int totalContents, int pPage, int numPerPage, String url ){
		...
		//[이전]section
		if(pageNo == 1 ){
			pageBar += "<li class='paginate_button page-item first disabled'>";
		...
		while(!(pageNo>pageEnd || pageNo > totalPage)){
			if(pPage == pageNo ){
				pageBar += "<li 'paginate_button page-item first disabled'>";
		...
		//[다음] section
		if(pageNo > totalPage){
			pageBar += "<li paginate_button page-item next>";
		...
		pageBar += "location.href='/bunny/"+url+"&pPage='+pPage;";
		pageBar += "}";
		pageBar += "</script>";
